...
후속 확인에서는 위 주석의 정확한 문맥(`파주시 스마트 전자지도(paju.noblapp.com)로 업소 상세내용 확인 가능`)을 오늘의 PAJU-1001 직접 캡처에서 다시 확인한 뒤, 그 URL 하나만 read-only로 요청했다.

| 요청 URL | HTTP capture | 최종 렌더링 host | 접근·상세 구조 결과 |
| --- | --- | --- | --- |
| `https://paju.noblapp.com/` | `2026-09-25T11:08:47.7706216Z`; HTTP `200`; `text/html;charset=UTF-8`; HTTP redirect `0`; body SHA-256 `4db77fec80d88bf8753720aaffee6a0cf2edbd5d7be2c09f38e3e98cb7d53e02` | 정상 브라우저 렌더링 후 `https://www.noblapp.com/` | 로그인·인증·API key 요구는 없었지만, 결과는 노블앱의 일반 “스마트 관광 키오스크 솔루션” 회사 소개였다. 군장병·할인업소·파주 및 두 control 상호는 화면 text에 0건이고, 파주시 군장병 업체 목록/상세/검색 구조는 없었다. |

즉, 이 URL은 파주시 현재 공식 HTML에서 활성 연결이 아니며, 현재 렌더링 결과도 파주 데이터와 감사 가능한 연결을 제공하지 않는다. URL의 HTTP `200`은 업체별 상세 혜택 source의 존재를 뜻하지 않는다. 전체 사이트 검색·crawl·추측 URL 요청은 하지 않았다.
...
스마트지도 후보의 현재 렌더링 화면에서도 `두둑한한판`과 `게이트호텔`은 각각 0건이었다. 따라서 두 control의 존재 또는 업체별 혜택을 이 후보로 확인하지 못했다.
...
## 최종 판정 — `BLOCKED_EVIDENCE`

공식 파주시 목록과 집계 정책은 확인했지만, 업체별 상세 혜택 근거 또는 그 근거로 감사를 가능하게 하는 현재 활성 연결은 끝내 확인하지 못했다. 후속 확인한 `paju.noblapp.com`은 HTTP 응답 후 일반 노블앱 회사 사이트로 렌더링되며, 파주 군장병 할인업소·두 control·상세 혜택을 제공하지 않는다. 이전의 `NEW_CAPABILITY_REQUIRED` 결론은 이 후보가 현재 상세 source일 가능성을 남겨둔 잠정 판단이었으며, 이 후속 확인으로 대체한다.

따라서 `REUSE_PATH_IDENTIFIED`가 될 재사용 source가 없고, 단순히 새 adapter 또는 multi-source capability를 추가해도 공식 업체별 detail evidence를 만들 수 없다. 활성 공식 상세 source 또는 명시적·감사 가능한 공식 연결이 확인될 때까지는 구현을 시작하지 않는다.

## 미확인 사항과 위험

- 관측은 위 UTC 시점의 직접 응답에 한정된다. 목록의 빈 rendered 기준일과 HTML 주석의 날짜는 현재성 증거가 아니다.
- `paju.noblapp.com`은 비활성 HTML 주석에만 있으며, 후속 direct probe의 최종 렌더링은 일반 노블앱 회사 사이트였다. 이후 활성 공식 링크가 새로 생기더라도 공식성·redirect·현재성 검증을 다시 해야 한다.
...
파주시가 업체별 상세 혜택을 제공하는 현재 활성 공식 source 또는 그 source로 가는 명시적·감사 가능한 연결을 제공할 때까지 A2 구현과 A3를 시작하지 않는다. 그 전에는 adapter, multi-source capability, 데이터 변경을 승인하거나 구현하지 않는다.
