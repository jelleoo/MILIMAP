...
이 문서는 세 명의 팀원이 동시에 작업하더라도 같은 브랜치나 파일을 중복 수정하지 않도록 Issue, 파일 예약, 병렬 작업과 병합 순서를 정의합니다.
...
- 교차 리뷰는 기본 필수 gate가 아닙니다. Issue에서 요구하거나 별도 합의한 경우에만 필수입니다.
- 리뷰 여부와 별개로 테스트, CI, diff, 데이터 검증, 공용 파일 충돌 검사는 필요한 범위에서 수행합니다.
...
저장소 변경을 시작할 때 다음을 확인합니다.

1. 최신 `origin/dev`를 확인합니다.
2. `AGENTS.md`와 `docs/current-work.md`를 읽습니다.
3. Issue의 목표, 범위, 제외 범위, 수정 예정 경로와 완료 조건을 확인합니다.
4. 다른 활성 Issue가 같은 파일을 예약하지 않았는지 확인합니다.
5. 최신 `origin/dev`에서 Issue 전용 브랜치를 만듭니다.

조사나 검토처럼 저장소를 변경하지 않는 작업은 불필요한 branch/worktree 생성을 강제하지 않습니다.
...
- 두 사람이 하나의 브랜치를 공유하지 않습니다.
- 같은 작업 폴더에서 충돌 가능한 병렬 작업을 동시에 실행하지 않습니다.
...
PR 병합 전 최신 `dev`를 반영하고 변경 성격에 필요한 검증을 수행합니다.
...
공용 파일에서 충돌이 발생하면 파일 전체에 `ours` 또는 `theirs`를 임의로 선택하지 않습니다. 두 Issue의 목적과 계약을 확인하고 통합 담당자가 해결합니다. 팀 합의 없이 `--force` 또는 `--force-with-lease` push를 사용하지 않습니다.

## 리뷰와 병합

교차 리뷰는 기본 필수 조건이 아닙니다. Issue가 요구하거나 별도 합의한 경우에만 필수입니다.

병합 전에는 변경에 해당하는 다음 gate를 확인합니다.

- Issue 범위 밖 변경 없음
- 예약된 파일 경계 준수
- 필요한 로컬 테스트 통과
- 해당되는 GitHub CI 통과
- 데이터 변경 시 생성 데이터/정본/seed count 및 diff 검증
...
AI 도구와 프롬프트 형식은 팀원이 상황에 맞게 선택할 수 있으며, 공통 AI 안전 원칙은 `docs/ai-development.md`를 따릅니다.
