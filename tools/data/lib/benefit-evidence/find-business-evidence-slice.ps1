...
function Find-BenefitBusinessEvidence {
    ...
    if ($strong.Count -eq 1 -and $unexcluded.Count -eq 1) {
        $selected = $strong[0]
        $slice = New-RelevantBenefitEvidenceSlice -Observation $Observation -Unit $selected.Unit -IdentityEvidence $selected.Evidence -HtmlTokens $htmlTokens -HtmlValidationIndex $htmlValidationIndex -XlsxValidationIndex $XlsxValidationIndex
