...
function Get-NormalizedNameParts {
 ...
 if ($m.Success) { $base=$m.Groups['base'].Value.Trim(); $branch=$m.Groups['branch'].Value.Trim() }
  ...
  if ($m.Success) { $base=$m.Groups['base'].Value.Trim(); $branch=$m.Groups['branch'].Value.Trim() }
   ...
   if ($m.Success) { $candidate=$m.Groups['branch'].Value; $stem=$candidate.Substring(0,$candidate.Length-1); $hint=(@($LocalityHints|ForEach-Object { ([string]$_ -replace '(시|구|군|동|읍|면)$','') }) -join ''); if ($hint -and $stem -eq $hint) { $base=$m.Groups['base'].Value.Trim(); $branch=$candidate } elseif ($candidate -notin @('전문점','음식점','상점','매장')) { $warnings+='BRANCH_UNCERTAIN' } }
  }
 }
...
function Get-AdministrativeAddressParts {
 ...
 foreach($t in ($text -split '[\s,()]+'|Where-Object{$_})) { if(-not $city -and $t -match '시$'){ $city=$t;continue }; if(-not $district -and $t -match '(구|군)$'){ $district=$t;continue };if(-not $dong -and $t -match '(동|읍|면)$'){ $dong=$t } }
...
function Get-NormalizedAddressParts {
 ...
 if($road){
  ...
  if($road -match '[~]|\d+호\s*,\s*\d+호' -or $floors.Count -gt 1 -or $units.Count -gt 1){$ambiguous=$true;$warnings+='FLOOR_UNIT_UNCERTAIN'}else{if($floors.Count -eq 1){$floor=($floors[0].Groups[1].Value -replace '\s+','')};if($units.Count -eq 1){$unit=($units[0].Groups[1].Value -replace '\s+','').ToUpperInvariant()}}
 }
