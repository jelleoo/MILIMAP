function New-XlsxTestBytes {
    param(
        ...
        [switch]$DuplicateBusinessNameCell,[switch]$FormulaBenefit,[switch]$UnrelatedUnsupportedRow,[switch]$UnmappedUnsupportedCell,[switch]$NonCandidateFormulaBenefit,[switch]$NonCandidateUnsupportedBenefit,[switch]$DuplicateBenefitHeader,[switch]$MergedBusinessHeader,
    ...
    try {
        ...
        $primaryRow = '<row r="22"><c r="' + $(if($MalformedCellRow){'B23'}else{'B22'}) + '" t="s"><v>4</v></c>' + $(if($DuplicateCell){'<c r="B22" t="s"><v>4</v></c>'}else{''}) + '<c r="C22" t="s"><v>5</v></c><c r="D22" t="s"><v>6</v></c>' + $benefitCell + $(if($UnmappedUnsupportedCell){'<c r="H22" t="b"><v>1</v></c>'}else{''}) + '</row>'
        $parts=[ordered]@{
